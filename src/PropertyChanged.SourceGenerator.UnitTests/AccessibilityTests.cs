...
public class AccessibilityTests : TestsBase
{
    ...
    public void HandlesInternalGetterProtectedSetter()
    {
        ...
        this.AssertThat(input, It.HasFile("SomeViewModel", StandardRewriters)
            .HasDiagnostics(
            // (3,6): Warning INPC005: C# propertes may not have an internal getter and protected setter, or protected setter and internal getter. Defaulting both to protected internal
            // Notify(Getter.Internal, Setter.Protected)
            Diagnostic("INPC005", @"Notify(Getter.Internal, Setter.Protected)").WithLocation(3, 6)
        ));
    }
    ...
    public void RaisesIfBackingFieldIsReadOnly()
    {
        ...
        this.AssertThat(input, It.HasFile("SomeViewModel", StandardRewriters)
            .HasDiagnostics(
                // (4,26): Warning INPC018: Backing field '_foo' cannot be readonly. Skipping
                // _foo
                Diagnostic("INPC018", @"_foo").WithLocation(4, 26)
        ));
    }
    ...
    public void RaisesIfBackingPropertyIsGetterOnly()
    {
        ...
        this.AssertThat(input, It.HasFile("SomeViewModel", StandardRewriters)
            .HasDiagnostics(
                // (4,17): Warning INPC019: Backing property '_foo' cannot be getter-only. Skipping
                // _foo
                Diagnostic("INPC019", @"_foo").WithLocation(4, 17)
        ));
    }
    ...
    public void RaisesIfBackingPropertyIsSetterOnly()
    {
        ...
        this.AssertThat(input, It.HasFile("SomeViewModel", StandardRewriters)
            .HasDiagnostics(
                // (4,17): Warning INPC019: Backing property '_foo' must have a getter and a setter. Skipping
                // _foo
                Diagnostic("INPC019", @"_foo").WithLocation(4, 17)
        ));
    }
