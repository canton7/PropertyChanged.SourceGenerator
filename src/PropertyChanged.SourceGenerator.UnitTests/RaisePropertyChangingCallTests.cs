...
public partial class SomeViewModel : INotifyPropertyChanging
{
    ...
    private string _foo;
        ...
        this.AssertThat(input, It.HasDiagnostics(
            // (3,22): Warning INPC029: Found one or more methods called 'OnPropertyChanging' to raise the PropertyChanging event, but they had an unrecognised signatures or were inaccessible. No PropertyChanging events will be raised from this type
            // SomeViewModel
            Diagnostic("INPC029", @"SomeViewModel").WithLocation(3, 22)
        ));
    }
...
public partial class Derived : Base
{
    ...
    protected override void OnPropertyChanging(string name) { }
        ...
        this.AssertThat(input, It.HasDiagnostics(
            // (12,29): Warning INPC032: Method 'OnPropertyChanging' must not be overridden. Functionality such as dependencies on base properties will not work. Define a method called 'OnAnyPropertyChanged' instead
            // OnPropertyChanging
            Diagnostic("INPC032", @"OnPropertyChanging").WithLocation(12, 29)));
    }
